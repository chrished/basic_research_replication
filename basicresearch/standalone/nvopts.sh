...
#----------------------------------------------------------------------------
            ...
            CC='nvcc'
            CFLAGS='-O3 --ptxas-options=-v -arch sm_13 --compiler-bindir /home/doug/CUDA/bindir -Xcompiler "-fPIC -D_GNU_SOURCE -pthread -fexceptions -m32"'
...
#----------------------------------------------------------------------------
            RPATH="-Wl,-rpath-link,$TMW_ROOT/bin/$Arch"
            CC='nvcc'
            CFLAGS='-O3 --ptxas-options=-v -arch sm_13 --compiler-bindir /home/doug/CUDA/bindir -Xcompiler "-fPIC -D_GNU_SOURCE -pthread -fexceptions -m64"'
            CLIBS="$RPATH $MLIBS -lm -lstdc++ -L$CUDA_ROOT/lib64"
            COPTIMFLAGS='-Xcompiler "-O3 -funroll-loops -msse2 -DNDEBUG"'
            CDEBUGFLAGS='-g'
#           
            CXX='g++'
