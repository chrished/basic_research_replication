...
__device__ inline unsigned int dlcg(unsigned int seed) {
...
__global__ void RandomGPU(
    ...
    for(int iRng = tid; iRng < MT_RNG_COUNT; iRng += THREAD_N){
        ...
        // reseed for next time using LCG from NR
        ds_MT[iRng].seed = dlcg(config.seed);
        ...
        for(iOut = 0; iOut < NPerRng; iOut++){
            ...
            d_Random[iRng + iOut * MT_RNG_COUNT] = ((double)x + 1.0f) / 4294967296.0f;
        }
    }
}
